...
open class ChartUtils {
    ...
    open class func textSize(_ text: String, unit: String?, font: UIFont) -> CGSize {
        let minFont = UIFont.systemFont(ofSize: 8.0)
        let att1 = NSAttributedString(string: text, attributes: [NSFontAttributeName: font])
        let att2 = NSAttributedString(string: (unit ?? ""), attributes: [NSFontAttributeName: minFont])
    ...
    /**
     ...
     - returns: The size of the string if it were drawn with a given font
     */
    open class func textSize(_ text: NSAttributedString, font: UIFont) -> CGSize {
        
        return NSAttributedString(string: text.string, attributes: [NSFontAttributeName: font]).size()
    }
