...
open class ChartAxisValue: Equatable, CustomStringConvertible {
    ...
    open var labels: [ChartAxisLabel] {
        let axisLabel = ChartAxisLabel(text: self.description, unit: self.unit, settings: self.labelSettings)
    ...
    public init(scalar: Double, unit: String, labelSettings: ChartLabelSettings = ChartLabelSettings()) {
        self.unit = unit
        self.scalar = scalar
        self.labelSettings = labelSettings
    }
