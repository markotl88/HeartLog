...
open class ChartPointsViewsLayer<T: ChartPoint, U: UIView>: ChartPointsLayer<T> {
    ...
    public typealias ChartPointViewGenerator = (_ color: UIColor, _ chartPointModel: ChartPointLayerModel<T>, _ layer: ChartPointsViewsLayer<T, U>, _ chart: Chart) -> U?
    public typealias ViewWithChartPoint = (view: U, chartPointModel: ChartPointLayerModel<T>)
    fileprivate var pointColor : UIColor = UIColor.blue
    ...
    public init(xAxis: ChartAxisLayer, yAxis: ChartAxisLayer, innerFrame: CGRect, pointColor: UIColor=UIColor.blue, chartPoints:[T], viewGenerator: @escaping ChartPointViewGenerator, conflictSolver: ChartViewsConflictSolver<T, U>? = nil, displayDelay: Float = 0, delayBetweenItems: Float = 0) {
    ...
    fileprivate func generateChartPointViews(chartPointModels: [ChartPointLayerModel<T>], chart: Chart) -> [ViewWithChartPoint] {
        let viewsWithChartPoints: [ViewWithChartPoint] = self.chartPointsModels.flatMap { model in
            if let view = self.viewGenerator(self.pointColor, model, self, chart) {
