//
//  GraphTableCellLandscape.swift
//  FTN
//
//  Created by Marko Stajic on 1/4/17.
//  Copyright © 2017 DMS. All rights reserved.
//

import UIKit

class GraphTableCellLandscape: UITableViewCell {
...
extension GraphTableCellLandscape {
    func fillXValuesArray(period: GraphPeriod){
        ...
        var pastDate = Date()
        xValues.append(createDateAxisValue(isFirstOrLast: true, dateStr: self.getGraphString(date: endDate), readFormatter: readFormatter, displayFormatter: displayFormatter))
        ...
        case .Week:
            ...
            for i in 0..<period.daysToShowOnX {
                ...
                if i == period.daysToShowOnX-1 {
                    xValues.append(createDateAxisValue(isFirstOrLast: true, dateStr: self.getGraphString(date: pastDate), readFormatter: readFormatter, displayFormatter: displayFormatter))
                }else{
                    xValues.append(createDateAxisValue(dateStr: self.getGraphString(date: pastDate), readFormatter: readFormatter, displayFormatter: displayFormatter))
                }
                endDate = pastDate
            }
        ...
        case .TwoWeeks:
            ...
            endDate = Date()
            xValues.append(createDateAxisValue(dateStr: self.getGraphString(date: endDate), readFormatter: readFormatter, displayFormatter: displayFormatter))
...
extension GraphTableCellLandscape : UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        let cellSize = CGSize(width: width/4.0, height: 50.0)
        return cellSize
    }
}
...
extension UITableViewCell {
    func getPopUpString(chartPoint: ChartPoint, unit: String)->String{
        var info = String(describing: chartPoint.y) + " " + unit
        if let dateStr = chartPoint.dateStr {
            info = info + "\n"
            info = [info,dateStr].flatMap{$0}.joined(separator: " ")
        }
        return info
    }
}
